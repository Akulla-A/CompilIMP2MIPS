...
let tr_function fdef =
   ...
   *)
    ...
    (** 
     ...
     obtained value in register $ti
   *)
   let rec getRegisterCount expr =
      match expr with
         | Cst(_) | Bool(_) | Var(_) -> 1
         | Binop(a, e1, e2) -> (getRegisterCount e1) + (getRegisterCount e2)
         | Call(_, params) -> 
            List.fold_right (fun e acc -> (getRegisterCount e) + acc) params 0 in
   ...
   let rec tr_exprAlt a i =
      ...
      | Var(id) -> begin
         match Hashtbl.find_opt env id with
            | Some offset -> lw reg offset fp
            | None -> la reg id @@ lw reg 0 reg
         end
      | Binop(bop, e1, e2) ->
